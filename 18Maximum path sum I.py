...
dum = "3 7 4 2 4 6 8 5 9 3"
path = list(map(int,a.split(" ")))
print(path)
result=path[0]
head = 1
index = 0
for i in range(2,len(path)):
    try:
        res = path[head:head+i]
        print(path[head:head+i])
        a = index - 1
        b = index + 1
        c = index
        if a != abs(a) :
            ...
            if res[a] > res[b] and res[a] > res[c]:
                index = a
            elif res[b] > res[c] and res[b] > res[c]:
                index = b
            else:
                index = c
            print(index)
